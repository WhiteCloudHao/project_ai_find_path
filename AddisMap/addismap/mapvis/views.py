...
def mapapp(request):
    if request.method == 'GET':
        ...
        context = {
            'message':message,
            'GMAPS_API_KEY': 'AIzaSyANxqe7hLug-NZc6VYdIjIiNgjb8NIdcng'
        }

    if request.method == 'POST':

        # osmPath = "/Users/yohannes/Developer/mapApp/addismap/mapvis/oak.osm"
        # osmPath = r"C:\Users\ASUS\OneDrive\computer\btl_ai\AddisMap\addismap\mapvis\oak.osm"
        # osmPath = r"C:\Users\ASUS\OneDrive\computer\btl_ai\AddisMap\addismap\mapvis\oak.osm"
        # osmPath = r"C:\Users\ASUS\OneDrive\computer\btl_ai\AddisMap\addismap\mapvis\map.osm"
        osmPath = r"C:\Users\ASUS\OneDrive\computer\btl_ai\AddisMap\addismap\mapvis\map_medium.osm"
        nodes = extract_osm_nodes(osmPath)
        edges = extract_osm_edges(osmPath)
        adj_list = adjacency_list(nodes, edges)
        print("handle a star 00 ")
        node = namedtuple('Node', ['lat', 'lng'])
        print("handle a star 0 ")

#####
        post = POST_parser(request)
        print("handle a star 0 1")
        ...
        if len(shortestPath)==0:
            request.session['message'] = "Sorry Couldn't Find Path. Please try different nodes. The Open Street Map data for Thanh Xuan, Ha Noi isn't completely filled with all the nodes as accurately as other modern cities."
            return redirect('/')
        else:
...
def showPath(request):
    ...
    context = {
        'GMAPS_API_KEY': 'AIzaSyANxqe7hLug-NZc6VYdIjIiNgjb8NIdcng',
        # 'GMAPS_API_KEY': 'AIzaSyAv0SjrNE-LMf6LncO5Lx40XP1VlGVCS6Q', //old api key of git
...
def addisMapapp(request):
    ...
    if request.method == 'POST':
        ...
        osmPath = r"C:\Users\ASUS\OneDrive\computer\btl_ai\AddisMap\addismap\mapvis\gerjisecond.osm"
        nodes = extract_osm_nodes(osmPath)
        edges = extract_osm_edges(osmPath)
        adj_list = adjacency_list(nodes, edges)
        node = namedtuple('Node', ['lat', 'lng'])
